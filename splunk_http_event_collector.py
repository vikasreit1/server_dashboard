...
class http_event_collector:
    def __init__(self, token, http_event_server, sourcetype=None, input_type='json', host="", http_event_port='8088',
                 http_event_server_ssl=True, max_bytes=_max_content_bytes):
        self.token = token
        self.batchEvents = []
    ...
    def sendEvent(self, payload, eventtime=""):
        ...
        if self.input_type == 'json':
            ...
            if 'sourcetype':
                payload.update({"sourcetype": self.sourcetype})
    ...
    def batchEvent(self, payload, eventtime=""):
        ...
        if ((self.currentByteLength + payloadLength) > self.maxByteLength or (
                    self.maxByteLength - self.currentByteLength) < payloadLength):
            if http_event_collector_debug:
    ...
    def batchThread(self, timeout=5):
        # Threads to send batches of events.

        start_time = time.time()

        while True or time.time() > start_time + 30:
            try:
                if http_event_collector_debug:
...
def main():
    ...
    http_event_collector_key_json = ""
    http_event_collector_key_raw = ""
    http_event_collector_host = ""

    testeventJSON = http_event_collector(http_event_collector_key_json, http_event_collector_host, http_event_port=5000)
    testeventRAW = http_event_collector(http_event_collector_key_raw, http_event_collector_host, 'raw', http_event_port=5000)
    ...
    for i in range(5):
        payload.update(
            {"event": {"action": "success", "type": "json", "message": "individual hello world", "event_id": i}})
    ...
    for i in range(50000):
        ...
        testeventRAW.batchEvent(
            "%s type=raw message=batch event_id=%s" % (time.strftime("%Y-%m-%d %H:%M:%S GMT", time.gmtime()), str(i)))
