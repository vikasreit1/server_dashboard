...
getResponse(){
        ...
        then
             ...
             generate_json $groupname $nodename $url $priority $node_status $node_color $ssh_status $ssh_color $telnet_status $telnet_color $dockerps_status $dockerps_color $overlay_status $overlay_color  $freeMem_status $freeMem_color $service_status $service_color $container_count $controller_status $controller_color
             continue 
        else
             check_dockerPS $host_name
             if [[ $dockerps_status == "DOWN" ]]; then
                 ...
                 generateMaildata $groupname $nodename $url $priority $node_status $node_color $ssh_status $ssh_color $telnet_status $telnet_color $dockerps_status $dockerps_color $overlay_status $overlay_color $freeMem_status $freeMem_color
                 generate_html $service_color $url $nodename $container_count
                 generate_json $groupname $nodename $url $priority $node_status $node_color $ssh_status $ssh_color $telnet_status $telnet_color $dockerps_status $dockerps_color $overlay_status $overlay_color  $freeMem_status $freeMem_color $service_status $service_color $container_count 
                 continue
             else
                 response=$(wget --secure-protocol=TLSv1  --timeout=20 --tries=1 --no-check-certificate $url:$portno 2>&1  | grep HTTP | tail -1 | cut -f6 -d" ")
             fi
...
# fi
#---------------------------------------------------
# Creating the HTML and JSON File for the dashboard
#---------------------------------------------------
...
cat $PREJSON >> ${HEALTHCHECK}_${cur_time}_json
...
cat $POSTJSON >> ${HEALTHCHECK}_${cur_time}_json
#-------------------------
